    ...
    return (
      <div className="flex min-h-screen items-center justify-center px-4 text-center">
        <div className="max-w-md rounded-3xl border border-brand-100 bg-white/90 p-10 text-neutral-700 shadow-lg dark:border-brand-500/20 dark:bg-slate-900/80 dark:text-neutral-200">
          <p className="text-2xl font-semibold">{language === 'es' ? 'Proyecto no encontrado' : 'Project not found'}</p>
          <button
            type="button"
            onClick={() => navigate('/portfolio-v.2/Projects')}
            className="mt-6 inline-flex items-center rounded-2xl bg-brand-600 px-5 py-2 text-sm font-semibold text-white shadow-brand transition hover:-translate-y-0.5 hover:bg-brand-700"
          >
    ...
    <section className="px-4 pb-24 pt-28">
      <div className="mx-auto w-full max-w-5xl space-y-10">
        <motion.div
          ...
          className="rounded-4xl border border-brand-100/70 bg-white/95 p-8 shadow-xl backdrop-blur dark:border-brand-500/20 dark:bg-slate-900/85"
        >
          <button
            type="button"
            onClick={() => navigate(-1)}
          ...
          <h1 className="mt-6 text-3xl font-semibold text-neutral-900 dark:text-white md:text-4xl">{project.title}</h1>
        ...
        {project.link && (
          ...
          >
            <div className="max-w-xl text-sm text-neutral-600 dark:text-neutral-300">
              {language === 'es'
                ? 'Explora el repositorio para revisar el codigo, decisiones tecnicas y roadmap.'
                : 'Explore the repository to review source code, technical decisions and the roadmap.'}
            </div>
            <a
              href={project.link}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-2xl bg-brand-600 px-5 py-2 text-sm font-semibold text-white shadow-brand transition hover:-translate-y-0.5 hover:bg-brand-700"
            >
              {language === 'es' ? 'Abrir repositorio' : 'Open repository'}
